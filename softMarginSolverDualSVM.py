...
# -a <= 0 (Old condition)
# a <= C (New condition)
G = -np.eye(numExamples, dtype=np.double) # For old condition (-a <= 0)
G = np.concatenate((G, np.eye(numExamples, dtype=np.double)), axis=0) # For new condition (a <= C)
h = np.zeros(numExamples, np.double)
h = np.full(numExamples * 2, C, np.double)
h[:numExamples] = 0.0
