...
if not os.path.exists("./liblinear/"):
	print ("Cloning Lib-Linear repository")
	from git import Repo # gitpython
	Repo.clone_from("https://github.com/cjlin1/liblinear.git", ".")
	print ("Repository sucessfully cloned!")
	print ("Please compile the Lib-Linear code by running make into the main directory as well as the python directory and execute the program again!")
	exit (-1)
...
pickleDir = './PickleDir-LibLinear'
weightsDir = './Weights-LibLinear'
logFile = open("LogFile-LibLinear.txt", "w")
plotOutputFileName = "Plot-LibLinear"
confMatOutputFileName = "ConfMat-LibLinear"
plotOutputLogFileName = open("plot-log-LibLinear.txt", "w")
...
def parallel(y_entry): 
	# Load the data
	y_entry = int(y_entry)
	fileName = os.path.abspath(os.path.join('./PickleDir-LibLinear', str(y_entry) + '_X.npz'))
	fileNameY = os.path.abspath(os.path.join('./PickleDir-LibLinear', str(y_entry) + '_Y.npy'))
	X = scipy.sparse.load_npz(fileName)
	y = numpy.load(fileNameY)

	# Call the lib-linear model
	model = train(y, X, '-s 3 -c 1 -q')
	weights = model.w
	bias = model.bias
	weights = numpy.ctypeslib.as_array(weights, shape=(numInputFeatures, 1))
	weights = numpy.expand_dims(numpy.append(weights, bias), -1)
	# print (numpy.count_nonzero(weights))
	
	# Original weights is the sum of the w_bar and the current computed weights
	weights += w_bar
	# print (weights.shape)

	if USE_SPARSE_MATRICES:
		weights = scipy.sparse.csc_matrix(weights) # For sparse vectors
		fileName = os.path.abspath(os.path.join('./Weights-LibLinear', 'w_' + str(y_entry) + '.npz'))
		scipy.sparse.save_npz(fileName, weights)
	else:
		fileName = os.path.abspath(os.path.join('./Weights-LibLinear', 'w_' + str(y_entry) + '.npy'))
...
for i in range(startingIteration, numIterations):
	try:
		...
		w_bar_new = numpy.zeros(w_bar.shape, numpy.float32)
		
		trainScores = []
		validationScores = []
		for y_entry in y_entries:
			if USE_SPARSE_MATRICES:
				fileName = os.path.abspath(os.path.join(weightsDir, 'w_' + str(int(y_entry)) + '.npz'))
				weightVector = scipy.sparse.load_npz(fileName)
			else:
				fileName = os.path.abspath(os.path.join(weightsDir, 'w_' + str(int(y_entry)) + '.npy'))
			...
			if USE_SPARSE_MATRICES:
				out = out.toarray()
			trainScores.append(numpy.squeeze(out))

			# Compute the validation set outputs
			out = X_val.dot(weightVector)
			if USE_SPARSE_MATRICES:
				out = out.toarray()
			validationScores.append(numpy.squeeze(out))
